...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen ${tm.bgApp} ${tm.textApp} transition-colors duration-500`}>
      ...
      >
        <div className="max-w-7xl mx-auto flex flex-row items-center justify-between gap-3 md:gap-8 w-full py-2">
          ...
          <div className="flex-1 flex items-center justify-end gap-2 md:gap-6 ml-1 md:ml-4">
            ...
            {/* Price Filter - Desktop only for space */}
      ...
      >
        ...
        >
          {[...specials, ...specials, ...specials].map((item, idx) => (
            <div 
              key={`${item.id}-${idx}`} 
              className="relative h-[148px] w-[300px] md:w-[500px] flex-shrink-0 group overflow-hidden rounded-xl border border-[#D4AF37]/10"
            >
              {item.image ? (
                <SafeImage 
                  src={item.image} 
      ...
      >
        <div className="max-w-7xl mx-auto flex flex-nowrap justify-start gap-3 md:gap-6 overflow-x-auto no-scrollbar py-2 w-full">
          {["All", ...menuData.map(c => c.category)].map((cat) => (
            <button
              key={cat}
              onClick={() => setActiveCategory(cat)}
              className={`px-5 py-2 md:px-8 md:py-2 rounded-full text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase transition-all border flex-shrink-0 whitespace-nowrap flex items-center justify-center gap-2 ${
                ...
                {section.items.map((item) => (
                  ...
                  >
                    ...
                    <div className="absolute -left-4 md:-left-8 w-44 h-44 md:w-60 md:h-60 flex-shrink-0 z-30 transition-transform duration-500 group-hover:scale-105">
                      <div className="relative w-full h-full rounded-full overflow-hidden border-2 border-[#D4AF37] shadow-[0_20px_50px_rgba(0,0,0,0.8)] bg-[#000000]">
                      {item.image ? (
                        <SafeImage 
                          src={item.image} 
                          alt={item[lang].name} 
                    ...
                    {/* Symmetrical Structured Frame Layer (Deep Black Shadows) */}
                    <div className={`w-full h-full bg-[#1A1A1A] rounded-3xl border border-[#D4AF37]/20 pl-40 md:pl-56 pr-8 md:pr-12 p-8 flex flex-col items-center justify-center text-center relative shadow-[0_25px_60px_rgba(0,0,0,1)] transition-all duration-500 hover:border-[#D4AF37]/40`}>
                      <div className="space-y-1.5 md:space-y-2 mb-4">
                        <h3 className={`text-xl md:text-3xl font-serif font-black text-[#D4AF37] transition-colors leading-tight break-words line-clamp-1`}>{item[lang].name}</h3>
                        <span className="text-[#D4AF37] font-black text-lg md:text-2xl whitespace-nowrap">${item.price}</span>
                        <p className={`text-white/60 text-[10px] md:text-sm italic font-light line-clamp-2 md:line-clamp-3 leading-relaxed transition-colors max-w-[200px] md:max-w-[300px]`}>{item[lang].desc}</p>
                      </div>
                      
                      <div className={`flex items-center justify-center gap-6 mt-2 pt-3 border-t border-[#D4AF37]/10 w-full max-w-[200px] md:max-w-[300px]`}>
                        <div className="flex items-center gap-2">
                          {cart[item.id] > 0 && !item.isSoldOut && (
                            <div className={`flex items-center gap-2 border border-[#D4AF37]/30 bg-black/20 rounded-full px-1.5 py-1 shadow-md`}>
                              <button 
                                onClick={() => removeFromCart(item.id)}
                                className={`w-6 h-6 flex items-center justify-center rounded-full text-xs transition-colors bg-[#D4AF37]/10 text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black`}
                              >
                                -
                              </button>
                              <span className={`text-[11px] font-black px-1 text-[#D4AF37]`}>{cart[item.id]}</span>
                              <button 
